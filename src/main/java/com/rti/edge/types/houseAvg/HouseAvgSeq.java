...
/**
* A sequence of HouseAvg instances.
*/
public final class HouseAvgSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public HouseAvgSeq() {
        super(HouseAvg.class);
    }

    public HouseAvgSeq (int initialMaximum) {
        super(HouseAvg.class, initialMaximum);
    }

    public HouseAvgSeq (Collection elements) {
        super(HouseAvg.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, HouseAvg.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' HouseAvg objects (beyond the original size of this object)
