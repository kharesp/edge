...
public class ParseData {
		...
		return Observable.create(observer -> {
			try {
				...
				System.out.format("Took %f seconds or %f mins or %f hours to read the file\n",
						(endTime - startTime) / (1000000000.0), (endTime - startTime) / (1000000000 * 60.0),
						(endTime - startTime) / (1000000000 * 60.0 * 60.0));
	...
	public static void splitHouseholdData(String filePath, String rootDir) {
		...
		Observable<GroupedObservable<Integer, Reading>> houseStreams = lines.map(l -> new Reading(l.split(",")))
			...
			houseStream.groupBy(r -> r.household_id).subscribe(houseHoldStream -> {
				...
				try {
					FileOutputStream fos = new FileOutputStream(
							new File(String.format(rootDir + "/%d/%d.csv", houseId, houseHoldId)), true);
	...
	public static void splitHouseData(String filePath, String rootDir) {
		new File(rootDir).mkdirs();
		...
		houseStreams.subscribe(houseStream -> {
			Integer houseId = houseStream.getKey();
			try {
				FileOutputStream fos = new FileOutputStream(new File(String.format(rootDir + "/%d.csv", houseId)),
						true);
				houseStream.subscribe(r -> {
					try {
						fos.write(r.toString().getBytes());
					} catch (IOException e) {
						e.printStackTrace();
	...
	public static void main(String args[]) {
		splitHouseData("/vagrant/DEBS2014/sorted100M.csv", "data");
	}
